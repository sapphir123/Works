    ...
    private struct TriggerJob :ITriggerEventsJob
    {
        ...
        public void Execute(TriggerEvent triggerEvent)
        {
            if (EnemyGroup.HasComponent(triggerEvent.EntityA))
            {
                ...
                #region 扣血并生成爆炸粒子实体
                if (HpGroup.HasComponent(triggerEvent.EntityA) && TargetGroup.HasComponent(triggerEvent.EntityB))
                {
            ...
            if (EnemyGroup.HasComponent(triggerEvent.EntityB) || CharacterGroup.HasComponent(triggerEvent.EntityB))
            {
                ...
                if (TranslationGroup.HasComponent(triggerEvent.EntityA) && BulletGroup.HasComponent(triggerEvent.EntityA))
                {
                    //是敌人射出的子弹且目标是敌人
                ...
                #region 扣血并生成爆炸粒子实体
                if (HpGroup.HasComponent(triggerEvent.EntityB) && BulletGroup.HasComponent(triggerEvent.EntityA))
                {
                    Bullet bullet = BulletGroup[triggerEvent.EntityA];
                    Hp hp = HpGroup[triggerEvent.EntityB];
                    hp.HpValue -= bullet.damage;
                    //Debug.Log("子弹击中" + bullet.damage);
                    //Debug.Log("是否是主角: " + CharacterGroup.HasComponent(triggerEvent.EntityB));
